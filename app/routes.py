...
def _disable_response_caching(response: Response) -> None:
    """Remove caching headers so browsers always request fresh data."""
...
def api_source_types():
    ...
    if df is None or df.empty:
        response = jsonify([])
        _disable_response_caching(response)
        return response
...
def api_archived_markers():
    ...
    response = jsonify(dataframe_to_markers(archived_df, include_archived=True))
    _disable_response_caching(response)
    return response
